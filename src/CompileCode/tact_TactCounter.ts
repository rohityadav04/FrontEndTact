...
export function loadAdd(slice: Slice) {
  let sc_0 = slice;
  if (sc_0.loadUint(32) !== 2335447074) {
    throw Error('Invalid prefix');
  }
  let _queryId = sc_0.loadUintBig(64);
...
const TactCounter_errors: { [key: number]: { message: string } } = {
...
export class TactCounter implements Contract {
  ...
  static async fromInit(id: bigint) {
    const init = await TactCounter_init(id);
    const address = contractAddress(0, init);
    return new TactCounter(address, init);
  }
